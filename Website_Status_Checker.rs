...
impl WebsiteStatus {
    fn save_to_file(&self) {
        ...
        let json_data = format!(
            "{{\n  \"url\": \"{}\",\n  \"action_status\": \"{}\",\n  \"response_time\": {},\n  \"timestamp\": {}\n}}",
            self.url, status_str, self.response_time, self.timestamp
        );

        let file_result = OpenOptions::new()
            .create(true)
            .append(true)
            .open("site_status_checked.json");
...
fn read_file() -> Vec<String> {
    let file = File::open("sites.txt").expect("Failed to open file");
...
fn check_website(url: &str, timeout: u64) -> WebsiteStatus {
    ...
    let status = match response {
        Ok(resp) => match resp.status() {
            StatusCode::OK => Ok(resp.status().as_u16()),
            _ => Err(resp.status().to_string()),
        },
